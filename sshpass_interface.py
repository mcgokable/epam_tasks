...
class SshpassUtility:
    def __init__(self, host, password, protocol='ssh', file_path=None):
        self.protocol = protocol
        self.host = host
        self.password = password
        self.file_path = file_path
        self.sshpass_command = self.do_command(self.password, self.file_path)

    def do_command(self, password=None, file_path=None):
        if password:
            return ['sshpass', '-p', self.password, self.protocol,
                    self.host]
...
class SCPUtility(SshpassUtility):
    def __init__(self, host, password, protocol='scp', file_path=None, ):
        super().__init__(host, password, file_path)
        self.protocol = protocol
        self.sshpass_command = self.do_command(password, file_path)

    def do_command(self, *args, password=None, file_path=None):
        if self.password:
            return ['sshpass', '-p', self.password, self.protocol]
...
if __name__ == '__main__':
    ...
    if args.protocol == 'scp':
        args_for_utility += [args.protocol, args.file_path]
